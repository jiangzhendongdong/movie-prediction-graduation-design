...
import config
import pymysql
from datasql.getAudienceFromDatabase import get_audience_from_database
from datasql.updateMovieId import update_movie_id
...
def crawl_data():
    def save_to_mysql(data_to_insert):
        ...
        try:
            ...
            if result == 0:
                ...
                sql_insert = ("INSERT INTO pre_movies (date, movie_title,movie_url, genre, country, audience) VALUES ("
                              "%s, %s, %s, %s, %s, %s)")
                cursor.executemany(sql_insert, data_to_insert)
            else:
                # 数据库不为空，根据电影名字自动更新audience
                for data in data_to_insert:
                    date, movie_title, movie_url, genre, country, audience = data
    ...
    for tr in trs:
        ...
        if len(tds) == 5:  # 确保找到了五个td元素
            ...
            movie_url = tds[1].find('a').get('href')
            genre = tds[2].text.strip()
            country = tds[3].text.strip()
            audience = tds[4].text.strip()

            # 将数据添加到列表中
            data_to_insert.append((date, movie_title, movie_url, genre, country, audience))
